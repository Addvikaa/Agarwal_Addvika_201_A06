...
[https://github.com/Addvikaa/Agarwal_Addvika_201_A06](https://github.com/Addvikaa/Agarwal_Addvika_201_A06)
...
```{r}
# Accessing LeBron James' points using indexing
lebron_points_indexing <- player_info$LJ$points
print(lebron_points_indexing)

# Accessing LeBron James' points using names
...
for (player_name in names(player_info)) {
  player_info[[player_name]]$height <- heights[player_name]
}
...
```{r}
# Initialize an empty list to store points per game for each player
...
for (player_initials in names(player_info)) {
    ...
    # Add points per game to the points_per_game_list with player's initials as name
    points_per_game_list[[player_initials]] <- points_per_game
}

# Print the points per game list
...
```{r}
# Use lapply to round each value to two decimal places
rounded_points_per_game <- lapply(points_per_game_list, round, digits = 2)

# Convert the result to a vector
rounded_points_per_game_vector <- unlist(rounded_points_per_game)

print(rounded_points_per_game_vector)
```
...
```{r}
mean_monthly <- apply(max_temps [,3:14], 2, mean)
...
max_ind <- which.max(annual_temps)
min_ind <- which.min(annual_temps)

max_year <- temps_CO$year[max_ind]
min_year <- temps_CO$year[min_ind]

print(paste0("The maximum median annual temperature of ", annual_temps[max_ind], " occured in ", max_year, " and the minimum median annual temperature of ", annual_temps[min_ind], " occured in ", min_year, "."))
```
...
sum_of_digits <- function(n) {
  ...
  # Take the last digit of the number input
  last_digit = n %% 10

  #Truncate thr number input 
  number_truncated = n %/% 10

  # conitnually add the last digit 
